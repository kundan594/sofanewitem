...
const Table = () => {
  ...
  const fetchDataInfo = DataAll?.news_items?.map((item, i) => (
    <tr className={i % 2 == 0 ? "bg-white" : "bg-gray-200"}>
      ...
      <td className="px-6 py-4  text-sm leading-5 text-gray-500">{item.tags}</td>

      <td className="px-6 py-4  text-sm leading-5 text-gray-500">{item.state}</td>
      <td className="px-6 py-4   text-sm leading-5 font-medium">
        <a
          href="#"
          className="text-indigo-600 hover:text-indigo-900  text-left text-xs"
        >
          {actionRender(item)}
        </a>
      </td>
        ...
        <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
          <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            <table className="min-w-full divide-y divide-gray-200">
              <thead>
                <tr>
                  <th className="px-6 py-3 flex-wrap bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                    Title
                  </th>
                  <th className="px-6 py-3 flex-no-wrap bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                    Description
                  </th>
                  <th className="px-6 py-3 flex-wrap bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider">
                    Category
                  ...
                  <th className="px-6 py-3 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider0">
                    {" "}
                    Actions
                  </th>
                </tr>
